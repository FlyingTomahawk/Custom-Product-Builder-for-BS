...
// CHECK IF OPTION EXISTS
            ...
            $q = "select products_options_id as o_next_id from products_options where products_options_name='" . mysql_real_escape_string($option_name) . "'"; 
            $res = tep_db_query($q);                               
...
// GET NEXT AVAILABLE OPTIONS & VALUES IDS
                $max_options_id_query = tep_db_query("select max(products_options_id) + 1 as o_next_id from products_options");
...
// ADD OPTION
                ...
                tep_db_query("insert into products_options (products_options_id, products_options_name, language_id) values ('" . (int)$products_options_id . "', '" . tep_db_input($option_name) . "', '" . (int)$languages_id . "')");
            }
...
// CHECK IF OPTIONS VALUE EXISTS
            $q = "select * from products_options_values where catalog_products_id=" . tep_db_prepare_input($products_count[$i]); 
            $res = tep_db_query($q);                               
            if(tep_db_num_rows($res)) {
                $ar = tep_db_fetch_array($res);
                ...
                $max_values_id_query = tep_db_query("select max(products_options_values_id) + 1 as v_next_id from products_options_values");
    ...
    // ADD OPTION VALUE - two new fields added to option_values table (pid & qty), these are used for returning components to stock when deleting builds                          
                ...
                tep_db_query("insert into products_options_values (products_options_values_id, language_id, products_options_values_name, catalog_products_id, catalog_products_quantity) values ('" . (int)$value_id . "', '" . (int)$languages_id . "', '" . $value_name . "', '" . tep_db_input($products_count[$i]) . "', '" . (int)$components_quantity[$i] . "')");
            }
            
// ADD NEW IDS TO ATTRIBUTE LIST (for direct to cart singles)
            $cpb_attribute_list[$next_options_id]=$next_values_id;
            
            $res = tep_db_query('select * from products_options_values_to_products_options where products_options_id=' . $option_id . ' and products_options_values_id=' . $value_id);            if(!tep_db_num_rows($res)) {                                    
...
// ADD ATTRIBUTE TO PRODUCT
            ...
            tep_db_query("insert into products_attributes values (NULL, '" . (int)$products_id . "', '" . (int)$options_id . "', '" . (int)$values_id . "', '" . tep_db_input($value_price) . "', '" . tep_db_input($price_prefix) . "')");
          }
// END OF THIS MODULE
?>
